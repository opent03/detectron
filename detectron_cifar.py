...
class CoolDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        l[i] = 1
        return torch.tensor(self.features[idx], dtype=torch.float32), torch.tensor(self.labels[idx], dtype=torch.long).squeeze(-1)
    
    
def load_pddm_cifar10():
    with open('pddm_cifar10.pkl', 'rb') as f:
        dct = pickle.load(f)
        #p_train, p_val, p_test_all = sample_data.cifar10(split='all')
        #q_all = sample_data.cifar10_1()

        p_train = CoolDataset(dct['x_train1'], dct['y_train1'])
        p_val = CoolDataset(dct['x_train2'], dct['y_train2'])
        p_test_all = CoolDataset(dct['x_val'], dct['y_val'])
        q_all = CoolDataset(dct['x_test'], dct['y_test'])
        return p_train, p_val, p_test_all, q_all
...
#                                    verbose=False)
load_model = lambda: MLP.load_from_checkpoint('pddm_cifar10.pth', input_size=512,
...
q_all = sample_data.cifar10_1()
'''
p_train, p_val, p_test_all, q_all = load_pddm_cifar10()
    ...
    for dataset_name in args.splits:
        for seed in range(args.seeds):
            ...
            for i in range(1, ensemble_size + 1):
                ...
                alpha = 5 / len(pq_loader.train_dataloader())
